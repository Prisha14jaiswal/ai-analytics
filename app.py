"""
Universal AI Analytics Assistant — Root Entry Point.
This file acts as the main entry point and synchronizes with the core logic in ai_analytics_app/.
"""
...
# ── Env & Path setup ─────────────────────────────────────────────────────────
# Ensure the ai_analytics_app directory is in the path so internal imports work
ROOT_DIR = Path(__file__).resolve().parent
APP_DIR = ROOT_DIR / "ai_analytics_app"
sys.path.insert(0, str(APP_DIR))

# Load .env
for env_path in [APP_DIR / ".env", ROOT_DIR / ".env"]:
    if env_path.exists():
...
if "df" in st.session_state:
    ...
    with t1:
        ...
        with col_b:
            # Fix: Ensure all inputs to pd.DataFrame are lists to avoid 'Mixing dicts' error
